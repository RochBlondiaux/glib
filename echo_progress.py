...
def get_progress_bar(numchars, fraction=None, percent=None):
    ...
    if fraction >= 1.0:
        return "\033[1;32m█\033[0m" * numchars
    ...
    return ("\033[1;32m█" * n_full + "\033[0m") + blocks[i_partial] + (" " * n_empty)
...
def main():
    ...
    fmt_str = "\r({:6.2f}%) [".replace("X", str(nchars))
    progress = 100 * args.stepno / args.nsteps
    sys.stdout.write(fmt_str.format(progress))
